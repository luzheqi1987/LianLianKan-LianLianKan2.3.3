...
public class HighlightView {
	...
	void handleMotion(int edge, float dx, float dy) {
		...
		if (edge == GROW_NONE) {
			...
			if (((GROW_LEFT_EDGE | GROW_RIGHT_EDGE) & edge) == 0) {
				dx = (((edge & GROW_TOP_EDGE) != 0) ? -1 : 1) * dy;
			} else if (((GROW_TOP_EDGE | GROW_BOTTOM_EDGE) & edge) == 0) {
				dy = (((edge & GROW_LEFT_EDGE) != 0) ? -1 : 1) * dx;
			} else {
				float t = Math.min(Math.abs(dx), Math.abs(dy));
				dx = dx > 0 ? t : -t;
				dy = dy > 0 ? t : -t;
			}
	...
	void growBy(float dx, float dy) {
		...
		if (mImageRect.height() < mImageRect.width()) {
			if (r.width() > r.height()) {
				r.inset((r.width() - r.height()) / 2, 0);
			}
		} else {
			if (r.height() > r.width()) {
				r.inset(0, (r.height() - r.width()) / 2);
			}
		}
