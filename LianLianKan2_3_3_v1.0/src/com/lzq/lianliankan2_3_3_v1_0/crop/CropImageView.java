package com.lzq.lianliankan2_3_3_v1_0.crop;
...
public class CropImageView extends ImageViewTouchBase {
	...
	protected void onLayout(boolean changed, int left, int top, int right,
		...
		if (mBitmapDisplayed.getBitmap() != null) {
			for (HighlightView hv : getmHighlightViews()) {
	...
	protected void postTranslate(float deltaX, float deltaY) {
		...
		for (int i = 0; i < getmHighlightViews().size(); i++) {
			HighlightView hv = getmHighlightViews().get(i);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN: // CR: inline case blocks.
			...
			for (int i = 0; i < getmHighlightViews().size(); i++) { // CR:
																// iterator
																// for; if
																// not, then
																// i++ =>
	...
	protected void onDraw(Canvas canvas) {
		super.onDraw(canvas);
		for (int i = 0; i < getmHighlightViews().size(); i++) {
			getmHighlightViews().get(i).draw(canvas);
		}
	}

	public void add(HighlightView hv) {
		getmHighlightViews().add(hv);
		invalidate();
	}

	public ArrayList<HighlightView> getmHighlightViews() {
		return mHighlightViews;
	}

	public void setmHighlightViews(ArrayList<HighlightView> mHighlightViews) {
